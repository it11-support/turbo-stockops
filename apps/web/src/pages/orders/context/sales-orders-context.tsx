...
interface SalesPersonContextType {
  ...
  pickLists: { value: string; label: string; notes: string }[]
  setPickLists: React.Dispatch<
    React.SetStateAction<{ value: string; label: string; notes: string }[]>
  >
  pickListForm: {
    picker?: string
    pickList?: string
  ...
  setPickListForm: React.Dispatch<
  ...
  rawOrders: any[]
  setRawOrders: React.Dispatch<React.SetStateAction<any[]>>
  selectedOrders: any[]
...
export default function SalesOrderProvider({ children }: Props) {
  ...
  const [pickListForm, setPickListForm] = useState<any>({
    picker: '',
    pickList: '',
    code: '',
    notes: '',
      ...
      subscribeWS<any>({
        ...
        callback: (incoming: any) => {
          if (filterRefs.current.length === 0) {
            ...
            const newIds = newOrders.map((o) => o.DocNum)
            ...
            setTotal((prevTotal) => {
              ...
              const newCount = newOrders
                .map((o) => o.DocNum)
                .filter((id) => !existingIds.has(id)).length
  ...
  useEffect(() => {
    subscribeWS<any>({
      ...
      callback: (incoming: any) => {
        ...
        const closedIds = closedOrders.map((o) => o.DocNum)

        // 1. Hapus dari rawOrders
        setRawOrders((prev) =>
          prev.filter((o) => !closedIds.includes(o.DocNum))
        )
        ...
        // 3 & 4. Hapus dari salesOrderIds & selectedIds
        setSalesOrderIds((prev) => prev.filter((id) => !closedIds.includes(id)))
        setSelectedIds((prev) => prev.filter((id) => !closedIds.includes(id)))

        // 5. Update total sales orders
        setTotal((prevTotal) => prevTotal - closedIds.length)

        // 6. Cek apakah semua masih dipilih
        setIsSelectAll(true)
      },
    })
  ...
  useEffect(() => {
    if (selectedIds) {
      const filteredSalesOrders = rawOrders.filter((so) =>
        selectedIds.includes(so.DocNum)
      )
  ...
  const fetchSalesOrders = async (loadMaster: boolean = false) => {
    ...
    try {
      ...
      setSalesOrders((prev) => {
        ...
        if (isFirstPage) {
          return [...newOrders] // new copy
        } else {
          const merged = [...prev, ...newOrders]
          const unique = new Map(merged.map((item) => [item.DocNum, item]))
          return [...unique.values()] // new array
        }
      })
      ...
      const ids = data.data?.salesOrderIds ?? []
  ...
  const fetchPickList = async () => {
    try {
      ...
      const options = pickLists.map((pickList: any) => ({
        value: pickList.id,
        label: pickList.code,
  ...
  const fetchOrderIds = async () => {
    try {
      ...
      const options = orderIds.map((orderId: any) => ({
  ...
  const fetchPickers = async () => {
    try {
      ...
      const options = pickers.map((picker: any) => ({
        value: picker.id,
        label: picker.name,
      }))
      setPickers(options)
