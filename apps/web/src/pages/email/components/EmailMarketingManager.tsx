...
const EmailMarketingManager: React.FC = () => {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        'http://localhost:10001/api/users/getall/all'
      )
  ...
  const renderToHtmlFallback = useCallback(
  ...
  const getEmailComponent = useCallback(
    (template: string, userEmail: string) => {
      switch (template) {
        case 'apple':
  ...
  const handleSendEmails = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description:
          'Failed to send emails. Please check the console for more details.',
        variant: 'destructive',
      })
    } finally {
      setIsLoading(false)
    ...
    <main className='grid flex-1 gap-4 p-4 md:gap-8 lg:grid-cols-3 xl:grid-cols-3'>
      <div className='grid auto-rows-max items-start gap-4 md:gap-8'>
        ...
        <div className='space-y-4'>
          <div>
            <Label htmlFor='users' className='dark:text-gray-300'>
          ...
          <div className='flex items-center space-x-2'>
            <Checkbox
              ...
              disabled={isLoading}
            />
            <Label htmlFor='sendToAll' className='dark:text-gray-300'>
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value='google'>
                  Google Play Policy Update Email
                </SelectItem>
          ...
          <div>
            <Label htmlFor='previewEmail' className='dark:text-gray-300'>
              Preview Email
            </Label>
            <Input
              id='previewEmail'
