...
export const columns: ColumnDef<PickListItem>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return <LongText className='max-w-50'>{row.getValue('Area')}</LongText>
    },
    enableSorting: false,
    meta: { className: 'w-50' },
  },
  ...
  },
  {
    ...
    header: ({ column }) => (
      <DataTableColumnHeader column={column} title='Printed At' />
    ),
    cell: ({ row }) => {
      const date = row.getValue('printed_at') as unknown as Date
